...
async function submitTemplate() {
  ...
  console.log('[Templates] Submitting payload:', payload);

  // List of endpoints to try in order. AiSensy's Stoplight docs lists this resource
  // under /direct-apis/t1 but doesn't expose the path in plain HTML.
  // We try the most likely path first and fall back automatically.
  const endpoints = [
    'https://backend.aisensy.com/direct-apis/t1/wa_template',
    'https://backend.aisensy.com/direct-apis/t1/template',
    'https://backend.aisensy.com/direct-apis/t1/wa-template'
  ];

  let lastError = null;
  let succeeded = false;
  const templateNameBefore = (payload.name || '').toLowerCase();

  for (const endpoint of endpoints) {
    console.log('[Templates] Trying endpoint:', endpoint);
    try {
      const req = {
        url: endpoint,
        headers: {
      ...
      console.log('[Templates] Raw response from', endpoint, ':', resp);

      // ZOHO.CRM.HTTP wraps the response. Parse it.
      let parsed = resp;
      if (typeof resp === 'string') {
        try { parsed = JSON.parse(resp); } catch (e) { parsed = { raw: resp }; }
      }
      console.log('[Templates] Parsed response:', parsed);

      // Zoho wraps it: { statusCode, headers, body }
      const httpStatus = parsed?.statusCode || parsed?.status || parsed?.code;
      let body = parsed?.body || parsed;
      if (typeof body === 'string') {
        try { body = JSON.parse(body); } catch (e) { /* keep as string */ }
      }
      console.log('[Templates] HTTP status:', httpStatus, '| Body:', body);

      // 404 = endpoint doesn't exist, try next
      if (httpStatus === 404 || (typeof body === 'string' && body.includes('Cannot POST'))) {
        lastError = 'Endpoint not found (404)';
        console.warn('[Templates] 404 at', endpoint, '— trying next');
        continue;
      }

      // Inspect body for errors
      const errMsg = body?.error?.message || body?.errors?.[0]?.message || body?.message;
      const isError = body?.error || body?.errors || (httpStatus && httpStatus >= 400);

      if (isError) {
        lastError = errMsg || `HTTP ${httpStatus}`;
        showToast('Failed: ' + lastError, 'error');
        console.error('[Templates] Submission error', body);
        succeeded = false;
        break; // don't retry other endpoints on a real error
      }

      // Look like success — but verify by refetching the template list
      console.log('[Templates] Apparent success, verifying...');
      const verified = await verifyTemplateExists(templateNameBefore);
      if (verified) {
        ...
        lastError = 'Endpoint returned success but template not found in list — likely wrong endpoint';
        console.warn('[Templates] Verification failed, trying next endpoint');
        continue;
      }
    } catch (e) {
      console.error('[Templates] Network/exception at', endpoint, ':', e);
      lastError = e.message || 'Network error';
    }
  }

  if (!succeeded && lastError) {
    showToast('Submission failed: ' + lastError, 'error');
    console.error('[Templates] All endpoints failed. Last error:', lastError);
    console.error('[Templates] If you see this, copy the console logs and share — we need to find the right URL/payload shape.');
  }
...
async function verifyTemplateExists(name) {
  try {
    const list = await fetchTemplates();
    const found = list.find(t => (t.name || '').toLowerCase() === name.toLowerCase());
    console.log('[Templates] Verification:', found ? 'FOUND' : 'NOT FOUND', 'looking for:', name);
    return !!found;
  } catch (e) {
    console.error('[Templates] Verification fetch failed', e);
